...
<body>
@php
    $labels = ['gold_gram'=>'گرم طلا','gold_mesghal'=>'مثقال طلا','silver_995_gram'=>'گرم نقره ۹۹۵','silver_995_mesghal'=>'مثقال نقره ۹۹۵','silver_9999_gram'=>'گرم نقره ۹۹۹.۹','silver_9999_mesghal'=>'مثقال نقره ۹۹۹.۹','quarter_coin'=>'ربع سکه','half_coin'=>'نیم سکه','full_coin'=>'تمام سکه'];
    $icons = ['gold_gram'=>'🥇','gold_mesghal'=>'⚖️','silver_995_gram'=>'🥈','silver_995_mesghal'=>'⚖️','silver_9999_gram'=>'🌕','silver_9999_mesghal'=>'⚖️','quarter_coin'=>'🪙','half_coin'=>'🪙','full_coin'=>'🪙'];
...
<main class="wrap">
    ...
    <section class="section grid">
        <article class="card"><h2>ثبت معامله</h2><p class="hint">دارایی، واحد و مقدار را انتخاب کنید. قیمت واحد را می‌توانید تغییر دهید؛ قیمت پیش‌فرض، قیمت سایت است.</p>
            <form id="trade-form"><div class="two"><div><label for="side">نوع معامله</label><select id="side" name="side"><option value="sell">خرید از سایت</option><option value="buy">فروش به سایت</option></select></div><div><label for="trade-asset">دارایی</label><select id="trade-asset" name="asset"><option value="gold">طلا</option><option value="silver_995">نقره ۹۹۵</option><option value="silver_9999">نقره ۹۹۹.۹</option><option value="full_coin">تمام سکه</option><option value="half_coin">نیم سکه</option><option value="quarter_coin">ربع سکه</option></select></div></div><div class="two"><div><label for="trade-unit">واحد</label><select id="trade-unit" name="unit"></select></div><div><label for="trade-quantity">مقدار</label><input id="trade-quantity" name="quantity" type="number" min="0.001" step="0.001" placeholder="مثلاً ۱۰"></div></div><label for="trade-price">قیمت واحد (ریال)</label><input id="trade-price" name="unit_price" type="number" min="1" step="1" placeholder="قیمت پیش‌فرض سایت"><label><input type="checkbox" name="allow_partial" value="1" checked style="width:auto;margin-left:6px"> پذیرش بخشی از معامله مجاز باشد</label><button type="submit">ثبت معامله</button><div class="result" id="trade-result"></div></form>
        </article>
        <article class="card"><h2>افزایش موجودی</h2><p class="hint">برای افزایش موجودی، ابتدا دارایی را به فروشگاه تحویل دهید.</p><button type="button" class="secondary" id="delivered-toggle">تحویل دادم</button>
            <form id="delivery-form" class="hidden"><div class="notice">پس از ثبت، درخواست تحویل شما برای بررسی فروشگاه ارسال می‌شود.</div><label for="delivery-asset">دارایی تحویلی</label><select id="delivery-asset" name="asset"><option value="gold">طلا</option><option value="silver_995">نقره ۹۹۵</option><option value="silver_9999">نقره ۹۹۹.۹</option><option value="full_coin">تمام سکه</option><option value="half_coin">نیم سکه</option><option value="quarter_coin">ربع سکه</option></select><label for="delivery-unit">واحد</label><select id="delivery-unit" name="unit"></select><label for="delivery-quantity">مقدار</label><input id="delivery-quantity" name="quantity" type="number" min="0.001" step="0.001"><button type="submit">ثبت تحویل به فروشگاه</button><div class="result" id="delivery-result"></div></form>
        </article>
...
<script>
const assets = {gold:'طلا',silver_995:'نقره ۹۹۵',silver_9999:'نقره ۹۹۹.۹',full_coin:'تمام سکه',half_coin:'نیم سکه',quarter_coin:'ربع سکه'};
const coinAssets=['full_coin','half_coin','quarter_coin'];
function setUnits(assetEl, unitEl, quantityEl, priceEl=null, allowMesghal=true) { const coin=coinAssets.includes(assetEl.value); unitEl.innerHTML=coin?'<option value="count">تعداد</option>':'<option value="gram">گرم</option>'+(allowMesghal?'<option value="mesghal">مثقال</option>':''); quantityEl.placeholder=coin?'تعداد سکه':'مقدار'; if(priceEl) setPrice(assetEl,unitEl,priceEl); }
function setMinimumQuantity(unitEl, quantityEl) { if (unitEl.value === 'gram') quantityEl.min='100'; else if (unitEl.value === 'mesghal') quantityEl.min='21.702'; else quantityEl.min='0.001'; }
...
setUnits(tradeAsset,tradeUnit,tradeQty,tradePrice,false);setMinimumQuantity(tradeUnit,tradeQty);tradeAsset.onchange=()=>{setUnits(tradeAsset,tradeUnit,tradeQty,tradePrice,false);setMinimumQuantity(tradeUnit,tradeQty)};tradeUnit.onchange=()=>{setMinimumQuantity(tradeUnit,tradeQty);setPrice(tradeAsset,tradeUnit,tradePrice)};
